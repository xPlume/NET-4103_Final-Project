...
def run_experiment(file_path, target_attr, removal_rate):
	...
	with torch.no_grad():
		...
		correct = preds.eq(labels[test_idx]).sum().item()
...
for attr in ATTRIBUTES:
	for rate in REMOVAL_RATES:
		...
		try:
			accuracy = run_experiment(FILE_NAME, attr, rate)
			final_results[(attr, rate)] = accuracy
...
# Final Summary Table
print("\n" + "="*45)
print(f"{'Attribute':<12} | {'Removed %':<10} | {'Accuracy':<10}")
print("-" * 45)

for (attr, rate), acc in final_results.items():
	print(f"{attr:<12} | {int(rate*100):<10}% | {acc:.4f}")
#for
